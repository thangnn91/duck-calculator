...
const Keypad = ({ actionProps }: { actionProps: ActionProps }) => {
  ...
  return (
    ...
    >
      {keys.map((key, idx) => (
        <Key key={idx} {...key} actionProps={actionProps} />
      ))}
    </div>
  );
};
...
const Key = ({ label, color, span, action, actionProps }: KeyProps) => {
  console.log("🚀 ~ file: Keypad.tsx:106 ~ Key ~ actionProps:", actionProps);
  ...
  useMemo(() => {
    if (!actionProps || !actionProps.keyContent || !actionProps.action) return;
    const { action, keyContent } = actionProps;
    if (action == "number") {
      if (calculator.result) {
        actions.setFirstValue(calculator.secondValue);
        actions.setSecondValue(keyContent.toString());
        actions.setResult("");
        return;
      }
      if (calculator.operator) {
      ...
      if (calculator.firstValue == "0") {
        actions.setFirstValue(keyContent.toString());
        return;
      }
      actions.setFirstValue(calculator.firstValue + keyContent);
    }
